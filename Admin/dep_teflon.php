...
// Quantity of teflon that Admin entered as depleted
$dep_teflon_quantity = (int)$_REQUEST['dep_teflon_quantity'];

    //  DELELTES one record 
$sql_query = "DELETE FROM TEFLON WHERE Date_Received IS NOT NULL order by Date_Received asc LIMIT 1";

echo "$sql_query";

// Deletes the exact quantity entered as depleted using a For loop
for ($x=0;$x<$dep_teflon_quantity;$x++){
    if(mysqli_query($conn,$sql_query)){
        echo "Teflon depleted successfully";
    }
    else{
